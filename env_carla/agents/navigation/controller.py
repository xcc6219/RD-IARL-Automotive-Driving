...
class VehiclePIDController():
    ...
    def run_step(self, target_speed, waypoint):
        ...
        if acceleration >= 0.0:
            control.throttle = float(min(acceleration, self.max_throt))
            control.brake = 0.0
        else:
            control.throttle = 0.0
            control.brake = float(min(abs(acceleration), self.max_brake))
...
class PIDLongitudinalController():
    ...
    def _pid_control(self, target_speed, current_speed):
        ...
        return float(np.clip((self._k_p * error) + (self._k_d * _de) + (self._k_i * _ie), -1.0, 1.0))
