...
class CarlaEnv:
    ...
    State vector (14-dim):
        0   prev_action_acc      [-1, 1]
        1   prev_action_steer    [-1, 1]
        2   angle_norm           heading deviation normalised [-1, 1]
        3   dev_norm             lateral deviation normalised [-1, 1]
        4   speed_norm           speed normalised [0, 2]
        5   a_long_norm          longitudinal accel normalised [-1, 1]
        6   light                red-light signal {0, 1}
        7   lead_dist_norm       distance to lead vehicle normalised [0, 1]
    ...
    def __init__(self, opt):
        ...
        self._prev_raw_action = np.zeros(2, dtype=np.float32)
    ...
    def step(self, action):
        ...
        # Store prev action BEFORE building obs so that the residual
        # formula in select_action sees the correct a_{t-1}, not a_{t-2}.
        self._prev_raw_action = np.array(
    ...
    def _get_obs(self):
        ...
        return {
            "state": ego_state,
            "birdseye": birdseye,
            "semantic": semantic_chw,
            "prev_action": self._prev_raw_action.copy(),
        }
    ...
    def _get_ego_state(self):
        ...
        target_v = max_v
        if self.is_turning:
            turn_ratio = min(1.0, abs(self.angle) / safe_max_angle)
            target_v = min(max_v * (1.0 - 0.7 * turn_ratio), 0.7 * max_v)

        if self.vehicle_front:
            d = float(self.vehicle_front_dist)
            if d < 10.0:
                target_v = 0.0
            elif d < 25.0:
                ramp = (d - 10.0) / 15.0
        ...
        lead_dist_norm = float(np.clip(self.vehicle_front_dist / 30.0, 0.0, 1.0))

        obs = np.array(
            [
                float(self._prev_raw_action[0]),     # 0  prev_action_acc   [-1,1]
                float(self._prev_raw_action[1]),     # 1  prev_action_steer [-1,1]
                angle_norm,                          # 2  angle_norm        [-1,1]
                dev_norm,                            # 3  dev_norm          [-1,1]
                speed_norm,                          # 4  speed_norm        [0,2]
                a_long_norm,                         # 5  a_long_norm       [-1,1]
                float(light),                        # 6  light             {0,1}
                lead_dist_norm,                      # 7  lead_dist_norm    [0,1]
                float(curvature_6[0]),               # 8  curv_lat_0        [-1,1]
                float(curvature_6[1]),               # 9  curv_lat_1        [-1,1]
                float(curvature_6[2]),               # 10 curv_lat_2        [-1,1]
                float(curvature_6[3]),               # 11 curv_head_0       [-1,1]
                float(curvature_6[4]),               # 12 curv_head_1       [-1,1]
                float(curvature_6[5]),               # 13 curv_head_2       [-1,1]
            ],
            dtype=np.float32,
        )
    ...
    def _terminal(self):
        ...
        if self.destinations is not None:
            if self._check_waypoint(self.destinations) and self._route_progress_ratio() >= float(self.goal_progress_ratio):
                return True
        return False
    ...
    def _transform_control(self, action):
        ...
        brake = max(-a_acc, 0.0) * max(float(self.acc_range), 0.6)
        steer_cmd = a_ste * float(self.steer_range)

        # Emergency brake override: force braking on red lights and close lead vehicles
        if bool(self.light_front) and self.forward_speed > 0.3:
            throttle = 0.0
            brake = max(brake, 0.8)
        if self.vehicle_front:
            d = float(self.vehicle_front_dist)
            if d < 10.0:
                throttle = 0.0
                brake = 1.0
            elif d < 15.0 and self.forward_speed > 2.0:
                throttle = 0.0
                brake = max(brake, 0.5 * (1.0 - (d - 10.0) / 5.0))
