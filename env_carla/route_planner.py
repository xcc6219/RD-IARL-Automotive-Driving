...
class RoutePlanner:
    ...
    def __init__(self, vehicle, town_map, buffer_size, destination=None,
        ...
        self._proximity_threshold = 30.0
    ...
    def _extend_route_to_length(self, plan_wp, plan_opt, target_len_m, step_dist=0.2, max_steps=20000):
        """Extend route by following waypoints until target length is reached.

        Includes loop detection: stops extending if the route loops back
        to the start point (within 15 m) after having extended far enough.
        """
        ...
        # Loop detection: record route start point
        start_loc = plan_wp[0].transform.location
        sx = float(start_loc.x)
        sy = float(start_loc.y)
        loop_proximity = 15.0
        ...
        for _ in range(int(max_steps)):
            ...
            # Loop detection: stop if route loops back to start
            cx = float(cand.transform.location.x)
            cy = float(cand.transform.location.y)
            d2_start = (cx - sx) ** 2 + (cy - sy) ** 2
            if cur_len > loop_proximity * 2 and d2_start < loop_proximity ** 2:
                break
    ...
    def _make_default_destination(self, start_location, forward_distance):
        """Walk forward along waypoints to find a default destination.

        Includes loop detection: if the route loops back near the start,
        return the farthest point from the start instead.
        """
        ...
        sx = float(start_location.x)
        sy = float(start_location.y)
        best_wp = cur
        best_d2 = 0.0
        loop_thresh = 15.0
        ...
        for _ in range(max_iter):
            ...
            cx = float(cur.transform.location.x)
            cy = float(cur.transform.location.y)
            d2 = (cx - sx) ** 2 + (cy - sy) ** 2
            ...
            # Loop detection: route looped back near start
            if dist > loop_thresh * 2 and d2 < loop_thresh ** 2:
    ...
    def _compute_closest_wp_and_progress(self):
        """Find closest waypoint on global plan with cursor monotonicity constraints."""
        ...
        # Limit search window to prevent cursor jumps on loop maps
        back = min(int(self._closest_search_back), 80)
        forward = min(int(self._closest_search_forward), 150)
        start = max(0, cur - back)
        end = min(n, cur + forward)
        ...
        # Monotonicity constraint: prevent large backward jumps or forward leaps
        max_jump = 80
        if best_idx < cur - back:
            best_idx = cur
        elif best_idx > cur + max_jump:
            best_idx = cur + max_jump
